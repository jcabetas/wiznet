...
//// Public Methods //////////////////////////////////////////////////////////////
//uint8_t writeReg(uint8_t addr, uint8_t dataSend)
//{
//    msg_t status;
//    uint8_t rx_data;
//    systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
//    i2cAcquireBus(&TOF_I2C);
//    status = i2cMasterTransmitTimeout(&TOF_I2C, addr, &dataSend, 1 , &rx_data, 0, tmo);
//    i2cReleaseBus(&TOF_I2C);
//    return status;
//}
...
//  last_status = bus->endTransmission();
  uint8_t rx_data, txdata[2];
  txdata[0] = reg;
  txdata[1] = value;
  systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 2 , &rx_data, 0, tmo);
  i2cReleaseBus(&TOF_I2C);
}
...
//  last_status = bus->endTransmission();
  uint8_t rx_data, txdata[3];
  txdata[0] = reg;
  txdata[1] = (uint8_t)(value >> 8); // value high byte
  txdata[2] = (uint8_t)(value); // value low byte
  systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 3 , &rx_data, 0, tmo);
  i2cReleaseBus(&TOF_I2C);
...
//  last_status = bus->endTransmission();
  uint8_t rx_data, txdata[5];
  txdata[0] = reg;
  txdata[1] = (uint8_t)(value >> 24); // value highest byte
  txdata[2] = (uint8_t)(value >> 16);
  txdata[3] = (uint8_t)(value >> 8);
  txdata[4] = (uint8_t)(value);       // value lowest byte
  systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 5 , &rx_data, 0, tmo);
  i2cReleaseBus(&TOF_I2C);
}
...
//  value = bus->read();
//  return value;
  uint8_t rx_data[5], txdata[1];
  txdata[0] = reg;
  systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 1 , rx_data, 1, tmo);
  i2cReleaseBus(&TOF_I2C);
  return rx_data[0];
}
...
//  return value;
  ...
  systime_t tmo = TIME_MS2I(500); // TIME_INFINITE; //
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 1 , rx_data, 2, tmo);
  i2cReleaseBus(&TOF_I2C);
  value  = (uint16_t)rx_data[0]<< 8; // value high byte
  value |=           rx_data[1];      // value low byte
  return value;
}
...
//
  ...
  txdata[0] = reg;
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 1 , rx_data, 4, TIME_MS2I(500));
  i2cReleaseBus(&TOF_I2C);
  value  = (uint32_t)rx_data[0] << 24; // value highest byte
  value |= (uint32_t)rx_data[1] << 16;
  value |= (uint16_t)rx_data[2] <<  8;
  value |=           rx_data[3];       // value lowest byte
  return value;
...
{
  uint8_t txdata[100];
  uint8_t *ptrData = (uint8_t *)src;
  if (count>sizeof(txdata)-1)
  {
      do {} while (1);
  }
  txdata[0] = reg;
  for (int16_t b=0;b<count;b++)
      txdata[b+1] = *(ptrData++);
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 1+count, NULL, 0, TIME_MS2I(500));
  i2cReleaseBus(&TOF_I2C);
}
...
//
  uint8_t txdata[1];
  uint8_t *ptrData = (uint8_t *)dst;
  txdata[0] = reg;
  i2cAcquireBus(&TOF_I2C);
  i2cMasterTransmitTimeout(&TOF_I2C, address, txdata, 1, ptrData, count, TIME_MS2I(500));
  //last_status = bus->endTransmission();
  i2cReleaseBus(&TOF_I2C);
}
