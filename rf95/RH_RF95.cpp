//https://github.com/PaulStoffregen/RadioHead/blob/master/RH_RF95.cpp
// RH_RF95.cpp
//
...
// devuelve true si hay mensaje recibido
bool RH_RF95::handleInterrupt()
{
    bool hayRxOk = false;
...
{
    chSysLock();  //    ATOMIC_BLOCK_START;
    _rxBufValid = false;
    _bufLen = 0;
    chSysUnlock(); //ATOMIC_BLOCK_END;
}
...
bool RH_RF95::send(const uint8_t* data, uint8_t len)
{
    ...
    waitPacketSent(200); // Make sure we dont interrupt an outgoing message (lo de 200 es mio)
    setModeIdle();
...
void RH_RF95::setModemRegisters(ModemConfigChoice index)//const ModemConfig* config)
{
    spiWrite(RH_RF95_REG_1D_MODEM_CONFIG1,  MODEM_CONFIG_TABLE[index][0]);//     config->reg_1d);
    spiWrite(RH_RF95_REG_1E_MODEM_CONFIG2,  MODEM_CONFIG_TABLE[index][1]);//     config->reg_1e);
    spiWrite(RH_RF95_REG_26_MODEM_CONFIG3,  MODEM_CONFIG_TABLE[index][2]);//     config->reg_26);
}
...
void RH_RF95::setLowDatarate()
{
    ...
    float symbolTime = 1000.0 * ((float) (1 << SF)) / bandwidth; // ms
